...
const ProgrammingProject: FC<programmingProjectProps> = ({
  ...
  useEffect(() => {
    ...
    if (isVisible) {
      ...
      const secondColonIndex = serverURL.indexOf(
        ":",
        serverURL.indexOf(":") + 1
      );
      const serverURLWithoutPort =
        secondColonIndex !== -1
          ? serverURL.slice(0, secondColonIndex)
          : serverURL;
      console.log(serverURLWithoutPort);
      iframe.src = `${serverURLWithoutPort}:${iframePort}/${iframePath}`;
